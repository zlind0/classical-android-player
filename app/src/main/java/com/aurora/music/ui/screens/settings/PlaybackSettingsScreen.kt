...
fun PlaybackSettingsScreen(contentPadding: PaddingValues, onBack: () -> Unit) {
    ...
    val strTitle = stringResource(R.string.playback_title)
    val strOutputSection = stringResource(R.string.playback_section_output)
    ...
    Ios5SettingsPage(strTitle, onBack) {
        ios5Section(strOutputSection) {
            Ios5SwitchRow(stringResource(R.string.playback_hires), stringResource(R.string.playback_hires_sub), prefs.preferHighRes) { v ->
        ...
        ios5Section(strPlaybackSection) {
            Ios5SliderRow(
                strCrossfade,
            ...
            Ios5CellDivider()
            Ios5SwitchRow(stringResource(R.string.playback_gapless), stringResource(R.string.playback_gapless_sub), prefs.gapless) { v -> scope.launch { store.setGapless(v) } }
            Ios5CellDivider()
            Ios5SwitchRow(stringResource(R.string.playback_skip_silence), stringResource(R.string.playback_skip_silence_sub), prefs.skipSilence) { v -> scope.launch { store.setSkipSilence(v) } }
            Ios5CellDivider()
            Ios5SwitchRow(stringResource(R.string.playback_mono), stringResource(R.string.playback_mono_sub), prefs.monoAudio) { v -> scope.launch { store.setMono(v) } }
        }

        ios5Section(strSpeedSection) {
            Ios5SliderRow(strSpeedLabel, "${"%.2f".format(prefs.defaultSpeed)}x", prefs.defaultSpeed, 0.5f..2.0f, steps = 5) { v ->
                scope.launch { store.setDefaultSpeed(v) }
            }
        }

        ios5Section(strAlarmSection) {
            Ios5SwitchRow(
                strAlarmTitle,
                strAlarmSub,
                alarm.enabled,
            ...
            Ios5CellDivider()
            Ios5NavRow(
                strAlarmTime, value = formatTime(alarm.hour, alarm.minute),
            ) {
...
private fun SignalPathCard() {
    ...
    Ios5Group(Modifier.padding(horizontal = 12.dp)) {
        Ios5StaticText(if (good) strBpTitle else strSignalTitle)
        Ios5CellDivider()
        Ios5StaticText("$parts  →  ${sp.output}")
        if (sp.note.isNotBlank()) {
            Ios5CellDivider()
            Ios5StaticText(sp.note)
        }
    }
}
